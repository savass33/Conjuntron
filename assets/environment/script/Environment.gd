...
func _on_EnemyLoader_timeout():
	...
	enemy.position = Vector2(_screen_size.x + 50, randf_range(35, _screen_size.y - 35))
	enemy.connect("destroy", Callable(self, "_on_Enemy_destroy"))
	add_child(enemy)
	_enemy_attack(enemy)
...
func _enemy_attack(enemy):
	var bullet = enemy.BulletScene.instantiate()
...
func _load_boss():
	...
	boss.connect("dead", Callable(self, "_on_boss_defeated"))
	add_child(boss)
	return boss
